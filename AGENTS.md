...
With **By load #** selected, the table below the chart mirrors the material table one row per outbound load: Load # · Customer PO · Ship Date · Materials · Stock Available · Needed for Loads · Variance, sorted worst-variance-first by default. Load # is the freight order from column P with its leading zeros stripped (`formatLoadNumber`), or an italic *No load #* when the export left it blank; the Customer PO goes through `formatCustomerPo`. Expanding a row lists that load's materials (Material · Name · Needed · Available · Variance) from the allocation's own `lines`, so pallet view converts with each material's footprint.
